...
const styles = {
    field: "w-full shadow-md bg-gray-100 border-b-4 transition hover:border-[#9ac9fb] ease-linear duration-300 my-2 p-4 outline-none transform hover:-translate-x-6", 
    errorsField: "w-full shadow-md bg-gray-100 border  border-red-500 my-2 p-4 outline-none",
          ...
          {({ errors, touched }) => (
            <Form className=" container mx-auto px-5" >
                        <div className=" w-full">
                            <Field className={`${errors.name && touched.name ? styles.errorsField : styles.field} h-16 `} name="name" placeholder="Nombre" type="text" />
                            <ErrorMessage component={ErrorComponent} name="name" />
                        </div>
                        <div className="w-full">
                            <Field className={`${errors.email && touched.email ? styles.errorsField : styles.field} h-16`} name="email" placeholder="Correo Electrónico" type="text" />
                            <ErrorMessage component={ErrorComponent} name="email" />
                        </div>
                        <div className="w-full">
                            <Field className={`${errors.phone && touched.phone ? styles.errorsField : styles.field} h-16`} name="phone" placeholder="Teléfono" type="tel" />
                            <ErrorMessage component={ErrorComponent} name="phone" />
                        </div>
                    <div className="w-full">
                            <Field
                            as="textarea"
                            className={`${errors.message && touched.message ? styles.errorsField : styles.field} h-32 resize-none`}
                            name="message"
                            placeholder="Mensaje"
                            />
                ...
                <div className="flex justify-center my-4">
                    <button className={`${styles.button} transform hover:scale-110 ease-in duration-300`} type="submit">
                    Enviar
                    </button>
                </div>
            </Form>
